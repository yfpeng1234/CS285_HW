...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            if params['do_dagger']:
                ...
                for i in range(len(paths)):
                    ...
                    action_by_expert=expert_policy.get_action(observation)
                    paths[i]['action']=action_by_expert
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          num_transition=params['train_batch_size']
          indices=np.random.permutation(replay_buffer.obs.shape[0])[0:num_transition]
        ...
        if log_metrics:
            ...
            #compute metrics for expert
            # ADDITIONAL
            eval_paths_expert, _ = utils.sample_trajectories_expert(
                env, expert_policy, params['eval_batch_size'], params['ep_len'])
            logs_expert=utils.compute_metrics_expert(eval_paths_expert)
            ...
            # ADDITIONAL
            for key, value in logs_expert.items():
                print('{} : {}'.format(key, value))
                logger.log_scalar(value, key, itr)
